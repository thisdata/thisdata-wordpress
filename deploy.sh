...
SVNURL="http://plugins.svn.wordpress.org/thisdata/" # Remote SVN repo on wordpress.org, with no trailing slash
SVNUSER="timfield" # your svn username
...
cp $SVNPATH/trunk/assets/* $SVNPATH/assets/
...
composer install --optimize-autoloader --no-dev --prefer-dist --ignore-platform-reqs
